...
namespace ListasSarlaft.Classes
{
    public class clsDtCadenaValor
    {
        ...
        public DataTable mtdConsultarNombreFiltro(ref string strErrMsg)
        {
            #region Vars
            cDataBase cDataBase = new cDataBase();
            DataTable dtInformacion = new DataTable();
            ...
            try
            {
                strConsulta = string.Format("	select distinct acm.UsuarioRegistra, RTRIM(LTRIM(CAST(lur.Nombres AS varchar))) + ' ' + RTRIM(LTRIM(CAST(lur.Apellidos AS varchar))) NombreUsuarioRegistra" +
                    " from Procesos.Acm acm LEFT JOIN Listas.Usuarios lur ON acm.UsuarioRegistra = lur.IdUsuario");
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta(strConsulta);
            }
            catch (Exception ex)
